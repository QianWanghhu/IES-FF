    ...
    while sample_step < num_steps:
        ...
        if sample_step >= 3:
            if (errors[sample_step - 2] <= 0.05) & (errors[sample_step-3] <= 0.05):
                ...
                thsd = [0.382, 0.0, 1]
                resample_flag = True
            elif (errors[sample_step - 2] > 0.1) | (errors[sample_step-3] > 0.1):
                print('----case 2-----')
                resample_flag = False
            else:
                print('----case 3-----')
...
def bayesian_inference_example():
    ...
    # init_scale = 30 # used to define length_scale for the kernel
    init_scale = 50 # used to define length_scale for the kernel
    num_vars = variables.nvars
    num_candidate_samples = 4000
    num_new_samples = np.asarray([20]+[8]*10+[16]*20+[24]*20+[40]*18)
    # num_new_samples = np.asarray([20]+[8]*10+[16]*20+[24]*10+[40]*10)
    ...
    kernel = RBF(length_scale, [(5e-2, 200), (5e-2, 200), (5e-2, 20), (5e-2, 10),
