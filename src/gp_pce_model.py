...
def resample_candidate(gp, sampler, gp_ob1=None, gp_ob2=None):
    ...
    if gp_ob1 is not None:
        y_temp1 = gp_ob1.predict(sampler.candidate_samples.T)
        index_temp1 = np.where(y_temp1 > np.sort(y_temp1, axis=0)[1000])
        # index_temp1 = np.argsort(y_temp1)[-1000:]
        x_select1 = sampler.candidate_samples[:, index_temp1[0]]
        x_max1 = x_select1.max(axis=1)

    if gp_ob2 is not None:
        y_temp2 = np.abs(gp_ob2.predict(sampler.candidate_samples.T))
        index_temp2 = np.where(y_temp2 < np.sort(y_temp2, axis=0)[1000])
        # index_temp2 = np.argsort(y_temp2)[0:1000]
        x_select2 = sampler.candidate_samples[:, index_temp2[0]]
        x_max2 = x_select2.max(axis=1)
    
    univariable_temp = [stats.uniform(0, min(x_max[ii], x_max1[ii], x_max2[ii])) \
        for ii in range(0, x_max.shape[0])]
    ...
    new_candidates = generate_candidate_samples(sampler.candidate_samples.shape[0],
        sampler.candidate_samples.shape[1], 
...
def convergence_study(kernel, function, sampler,
    ...
    while sample_step < num_steps:
        ...
        # evaluate candidate samples with GP narrow parameter range        
        new_candidates = resample_candidate(gp, sampler, gp_ob1, gp_ob2)
...
def bayesian_inference_example():
    ...
    num_new_samples = np.asarray([20]+[10]*6+[25]*6+[50]*8)
    ...
    kernel = RBF(length_scale, [(5e-2, 0.2), (5e-2, 0.2), (5e-2, 10), (5e-2, 5),
